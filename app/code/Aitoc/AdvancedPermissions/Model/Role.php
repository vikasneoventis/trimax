...
class Role extends \Magento\Framework\Model\AbstractModel implements RoleInterface
{
    ...
    {
        $this->_init('Aitoc\AdvancedPermissions\Model\ResourceModel\Role');
    }
    ...
    {
        return $this->getParam(self::SHOW_ALL_CUSTOMERS, 1);
    }
    ...
    {
        return $this->getParam(self::ALLOW_DELETE, 1);
    }
    ...
    {
        return $this->getParam(self::ALLOW_NULL_CATEGORY, 1);
    }
    ...
    {
        return $this->getParam(self::MANAGE_GLOBAL_ATTRIBUTE, 1);
    }
    ...
    public function getParam($field, $reverse = 0)
    {
        $value = $reverse;
        if ($this->getScope()) {
            $scope   = \Magento\Framework\App\ObjectManager::getInstance()->get(
                'Magento\Framework\App\Config\ScopeConfigInterface'
            )->getValue('advancedpermissions/general/' . $field);
            $useData = $this->getData('use_config_' . $field);
