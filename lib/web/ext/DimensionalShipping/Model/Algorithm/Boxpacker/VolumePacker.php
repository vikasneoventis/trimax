...
class VolumePacker implements LoggerAwareInterface
{
     ...
     */
    public function __construct(BoxInterface $box, ItemList $items)
    {
    ...
    public function pack()
    {
        ...
        while (!$this->items->isEmpty()) {
            ...
            $orientatedItem = $orientatedItemFactory->getBestOrientation($this->box, $itemToPack, $this->widthLeft, $this->lengthLeft, $this->depthLeft, $prevItem, $nextItem);
            ...
            if ($orientatedItem) {
                ...
                if ($this->widthLeft >= min($itemToPack->getWidth(), $itemToPack->getLength())
                    && $this->isLayerStarted(
                        $layerWidth,
                        $layerLength,
                        $layerDepth
                    ...
                    continue;
                } elseif ($this->lengthLeft < min($itemToPack->getWidth(), $itemToPack->getLength())
                    || $layerDepth == 0
                ) {
                ...
                $this->widthLeft  = $layerWidth ? min(floor($layerWidth * 1.1), $this->box->getInnerWidth())
                    : $this->box->getInnerWidth();
                $this->lengthLeft = $layerLength ? min(floor($layerLength * 1.1), $this->box->getInnerLength())
     ...
     * @return bool
     */
    protected function checkNonDimensionalConstraints(ItemInterface $itemToPack, ItemList $packedItems)
    {
        $weightOK = $itemToPack->getWeight() <= $this->remainingWeight;

        if ($itemToPack instanceof ConstrainedItemInterface) {
