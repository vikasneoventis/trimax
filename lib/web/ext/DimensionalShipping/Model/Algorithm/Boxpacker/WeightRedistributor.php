...
class WeightRedistributor implements LoggerAwareInterface
{
    ...
    public function redistributeWeight(PackedBoxList $originalBoxes)
    {
        ...
        $this->logger->log(
            LogLevel::DEBUG,
            "repacking for weight distribution, weight variance {$originalBoxes->getWeightVariance()}, target weight {$targetWeight}"
        );
        ...
        do { //Keep moving items from most overweight box to most underweight box
            ...
            $this->logger->log(
                LogLevel::DEBUG,
                'boxes under/over target: ' . count($underWeightBoxes) . '/' . count($overWeightBoxes)
            );
